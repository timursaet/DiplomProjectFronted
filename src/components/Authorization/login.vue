            ...
            <v-col cols="12" sm="8" md="4">
                <v-card class="elevation-12">
                    <v-toolbar color="primary" dark flat>
                        <v-toolbar-title>Войти {{errorView}}</v-toolbar-title>
        ...
        methods: {
            auth() {    
                this.$http.post('http://localhost:3000/login', {
                    ...
                    .then(response => {
                         const {err, status, token} = response.data;
                         localStorage.setItem('token', token)
                         if (status === 'manager') this.$router.push('manager/admin');
                            else this.$router.push('employee/admin');
                    })
